...
type ffmpegSourceConn struct {
	cancel context.CancelFunc
	cmd    *exec.Cmd
	done   chan struct{}
}

func (r *ffmpegSourceConn) Done() <-chan struct{} {
	return r.done
}

func (r *ffmpegSourceConn) Close() {
	if r.cancel != nil {
		r.cancel()
	}
...
func (r *Instance) connectFFmpegH264Source(ctx context.Context, args []string, label string, fps float64) (*ffmpegSourceConn, error) {
		...
		for {
			...
			if err != nil {
				if err != io.EOF && ffmpegCtx.Err() == nil {
					r.logger.Printf("%s read failed: %v", label, err)
				}
				break
			}
			if len(au) == 0 {
...
func ffmpegH264OutputArgs(cfg *Config) []string {
	args := []string{"-map", "0:v:0", "-an"}
